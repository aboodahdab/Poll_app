...
async function createVotingElementsHtml() {
  ...
  console.log(votingOptions, infoData);
  $votingForm.innerHTML = "";
  let query = "";
  for (let i = 0; i < votingOptions.length; i += 1) {
    query += `<label data-vote="voting-label" class="border-solid border-[1px] border-[rgb(213,204,215)] p-4 rounded-lg flex gap-3 items-center text-sm font-medium cursor-pointer transition duration-150ms cubic hover:bg-weird-red" id="" > <button class="h-4 w-4 border-solid border-[1px] border-black-700 rounded-full flex items-center justify-center" data-checked="unchecked" ></button >${votingOptions[i]}</label >`;
  }
  ...
  console.log($votingBtns);
}
async function makeNewVote(vote) {
...
$voteBtn.addEventListener("click", () => {
  ...
  const voteName = $element.textContent;
  makeNewVote(voteName);
});
